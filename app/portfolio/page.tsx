...
const projectPageArchiveSlugs = ["modernizing-alumni-operations", "fancy-car-wash", "car-wash-guys"];
              ...
              <div className="grid gap-0 lg:grid-cols-[0.82fr_1.18fr]">
                <div className="flex flex-col justify-between border-b border-foreground/[0.08] p-6 sm:p-8 lg:border-b-0 lg:border-r">
                  <div>
                    ...
                    <h2 className="mt-5 font-display text-6xl leading-none tracking-[-0.055em] text-foreground sm:text-7xl">Lumen</h2>
                  ...
                  <div className="mt-10 space-y-6">
                    <div className="grid grid-cols-2 gap-2">
                      {lumenSignals.map((signal) => (
                        <span key={signal} className="rounded-full border border-foreground/10 bg-background/45 px-3 py-2 text-center text-xs tracking-[0.12em] text-foreground/60">
                          {signal}
                        </span>
                      ))}
                    </div>
                    <a href={lumen.href} target="_blank" rel="noreferrer" className="inline-flex items-center gap-2 border-b border-foreground/30 pb-1 text-sm font-light text-foreground transition-colors hover:text-muted-foreground">
                ...
                <div className="relative min-h-[560px] overflow-hidden bg-[#f6efe4] p-4 sm:p-6 lg:min-h-[660px]">
                  ...
                  <div className="relative grid h-full gap-4 md:grid-cols-[0.86fr_1.14fr]">
                    <div className="relative min-h-[360px] overflow-hidden rounded-[1.75rem] border border-foreground/10 bg-background/70 shadow-card md:min-h-full">
                      <FallbackImage src={lumen.image} alt="Lumen interface preview" fill sizes="(min-width: 1024px) 35vw, 100vw" fallbackLabel="Lumen" imageClassName="object-cover object-center" />
                    </div>
                    <div className="relative min-h-[360px] overflow-hidden rounded-[1.75rem] border border-foreground/10 bg-background/70 shadow-card md:min-h-full">
                      <FallbackImage src={lumen.darkImage ?? lumen.image} alt="Muhome architecture preview" fill sizes="(min-width: 1024px) 45vw, 100vw" fallbackLabel="Muhome" imageClassName="object-cover object-center" />
                    </div>
                  </div>
                </div>
              </div>
          ...
          <div className="space-y-7 border-t border-foreground/[0.08] pt-14 sm:pt-16">
            <div className="flex flex-col justify-between gap-6 sm:flex-row sm:items-end">
              <div>
                ...
                <h2 className="mt-4 font-display text-4xl leading-none tracking-[-0.04em] text-foreground sm:text-5xl">Other case studies</h2>
              </div>
              <p className="max-w-md text-sm font-light leading-7 text-muted-foreground">
                Institutional systems and active local-business branding work kept intentionally concise.
              </p>
            </div>
            <div className="grid gap-5 lg:grid-cols-2">
              {selectedWork.map((project) => project && <ProjectCard key={project.slug} {...project} />)}
            </div>
          </div>
        </Container>
      </section>
